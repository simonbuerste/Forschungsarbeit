...
The field of unsupervised learning is a very fundamental one in machine learning. Since nearly every novel product is connected, a tremendous amount of data is generated. Neither every single data can be labelled, nor is it possible or necessary to save the whole original data, since most probably not everything is useful. Therefore, in the area of unsupervised learning, tasks like dimensionality reduction (sometimes also called representation learning) or clustering can help. Those two tasks are directly linked, since state-of-the art clustering methods use first a dimensionality reduction to one the one hand reduce the feature space but also extract relevant features out of the data. In most of these works a deep auto-encoder (or a variation of it) is trained to extract relevant features. Although in those works often a very good performance (in this case clustering accuracy) and generalizability is claimed, the experiments given in the publications are limited. Therefore different methods for clustering data are implemented and compared in this work inside a programmed framework within Tensorflow to allow a fair comparison of those methods. Therefore some state-of-the-art algorithms are investigated, implemented and optimized inside this framework with comparative model architecture and training procedure. These different models are then evaluated with different hyperparameters on different Datasets.
